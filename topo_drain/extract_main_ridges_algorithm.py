...
Input Requirements:
- Ridge Lines: Should have 'FID', 'TRIB_ID', and 'DS_LINK_ID' attributes (e.g., from Extract Ridges algorithm)
- Flow Accumulation Raster: Raster showing accumulated flow (e.g., from Extract Ridges algorithm, based on inverted DTM)
- Perimeter (optional): Polygon defining the study area boundary. If not provided, uses the extent of ridge lines"""
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if not ridge_lines_path or not os.path.exists(ridge_lines_path):
            raise FileNotFoundError(f"[Input Error] Ridge lines file not found: {ridge_lines_path}")
        ...
        if missing_attrs:
            raise ValueError(f"[Input Error] Ridge lines missing required attributes: {missing_attrs}. Please use output from Extract Ridges algorithm.")

        # Call the core function
        gdf_main_ridges = self.core.extract_main_ridges(
        ...
        gdf_main_ridges = gdf_main_ridges.set_crs(self.core.crs, allow_override=True)
        feedback.pushInfo(f"Main ridge lines CRS: {gdf_main_ridges.crs}")
        
        # Save result
        try:
            gdf_main_ridges.to_file(main_ridges_file_path)
            feedback.pushInfo(f"Main ridge lines saved to: {main_ridges_file_path}")
        except Exception as e:
            raise RuntimeError(f"[ExtractMainRidgesAlgorithm] failed to save main ridges output: {e}")
